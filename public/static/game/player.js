...
class PlayerProfile {
    constructor() {
        ...
        // Migrate old rocket-launch data to cosmic-runner
        this.migrateData();
    }

    // Migrate old data formats to new
    migrateData() {
        const levels = this.data.progress?.levels;
        if (!levels) return;

        let needsSave = false;

        // Rename rocket-launch to cosmic-runner if exists
        if (levels['rocket-launch'] && !levels['cosmic-runner']) {
            levels['cosmic-runner'] = levels['rocket-launch'];
            delete levels['rocket-launch'];
            needsSave = true;
            console.log('Migrated rocket-launch level data to cosmic-runner');
        }

        // Ensure cosmic-runner exists in levels (for profiles created before this level existed)
        if (!levels['cosmic-runner']) {
            levels['cosmic-runner'] = {
                unlocked: false,
                highScore: 0,
                bestStars: 0,
                timesPlayed: 0,
        ...
        // Migrate old achievement keys
        const achievements = this.data.achievements;
        if (achievements) {
            if (achievements['first_complete_rocket-launch'] && !achievements['first_complete_cosmic-runner']) {
                achievements['first_complete_cosmic-runner'] = achievements['first_complete_rocket-launch'];
                delete achievements['first_complete_rocket-launch'];
                needsSave = true;
            }
            if (achievements['perfect_rocket-launch'] && !achievements['perfect_cosmic-runner']) {
                achievements['perfect_cosmic-runner'] = achievements['perfect_rocket-launch'];
                delete achievements['perfect_rocket-launch'];
                needsSave = true;
            }
        }
    ...
    unlockNextLevel(currentLevelId) {
        const levelOrder = ['asteroid-defense', 'cosmic-runner', 'space-race', 'robot-builder'];
...
const ACHIEVEMENTS = {
    ...
    'first_complete_cosmic-runner': { name: 'Runner', description: 'Complete Cosmic Runner', icon: '🚀' },

    // Perfect scores
    'perfect_asteroid-defense': { name: 'Perfect Defense', description: 'Get 3 stars on Asteroid Defense', icon: '💫' },
    'perfect_cosmic-runner': { name: 'Perfect Runner', description: 'Get 3 stars on Cosmic Runner', icon: '🌟' },
