...
class CosmicTyper {
    ...
    render(currentTime) {
        ...
        // Draw HUD (only if level doesn't have its own)
        if (this.state === 'playing' && this.currentLevel) {
            const hudData = this.currentLevel.getHUD();
            if (!hudData.skipGlobalHUD) {
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        console.log('Initializing Cosmic Typing...');
        window.game = new CosmicTyper();
        console.log('Cosmic Typing initialized successfully!');
    } catch (error) {
