...
class AudioManagerClass {
    constructor() {
        ...
        this.audioFiles = {
            ...
            // NEW Level 2 sounds
            shortLaser: '/static/audio/short_laser_fire.wav',
            healthUp: '/static/audio/health_up.wav',
            metallicImpact: '/static/audio/metallic_impact.wav',
            metallicImpact2: '/static/audio/mettalic_impact_2.wav',
            scatterLaser: '/static/audio/scatter_laser_shot.wav',
            powerUpNew: '/static/audio/power_up.wav',
            typeLock: '/static/audio/type_lock.wav',
            bossDestruction: '/static/audio/boss_destruction.wav',
    ...
    async preloadSoundEffects() {
        const effectFiles = {
            ...
            // NEW Level 2 sounds
            shortLaser: this.audioFiles.shortLaser,
            healthUp: this.audioFiles.healthUp,
            metallicImpact: this.audioFiles.metallicImpact,
            metallicImpact2: this.audioFiles.metallicImpact2,
            scatterLaser: this.audioFiles.scatterLaser,
            powerUpNew: this.audioFiles.powerUpNew,
            typeLock: this.audioFiles.typeLock,
            bossDestruction: this.audioFiles.bossDestruction
        };
    ...
    // === NEW LEVEL 2 SOUND EFFECTS ===

    // Short laser fire - for basic projectile shots
    playShortLaser() {
        if (!this.enabled || !this.sfxEnabled) return;
        if (this.soundsLoaded && this.soundBuffers.shortLaser) {
            this.playBuffer('shortLaser', 0.6);
        } else {
            this.playLaser(); // Fallback
        }
    }

    // Health up - when collecting health bonus
    playHealthUp() {
        if (!this.enabled || !this.sfxEnabled) return;
        if (this.soundsLoaded && this.soundBuffers.healthUp) {
            this.playBuffer('healthUp', 0.8);
        } else {
    ...
    // Metallic impact - for hitting obstacles (with variety!)
    playMetallicImpact() {
        if (!this.enabled || !this.sfxEnabled) return;
        if (this.soundsLoaded) {
            // Randomly choose between two impact sounds for variety
            const useAlt = Math.random() < 0.5 && this.soundBuffers.metallicImpact2;
            if (useAlt) {
                this.playBuffer('metallicImpact2', 0.7);
            } else if (this.soundBuffers.metallicImpact) {
                this.playBuffer('metallicImpact', 0.7);
            }
        } else {
            this.playExplosion('small'); // Fallback
        }
    }

    // Power up new - enhanced power up sound
    playPowerUpNew() {
        if (!this.enabled || !this.sfxEnabled) return;
        if (this.soundsLoaded && this.soundBuffers.powerUpNew) {
            this.playBuffer('powerUpNew', 0.85);
        } else {
            this.playPowerUp(); // Fallback to synthesized version
        }
    }

    // Type lock - when locking onto a boss word
    playTypeLock() {
        if (!this.enabled || !this.sfxEnabled) return;
        if (this.soundsLoaded && this.soundBuffers.typeLock) {
            this.playBuffer('typeLock', 0.75);
        } else {
            this.playTone(440, 0.1, 'sine', 0.25);
            setTimeout(() => this.playTone(660, 0.08, 'sine', 0.2), 50);
        }
    }

    // Boss destruction - big explosion for defeating a boss
    playBossDestruction() {
        if (!this.enabled || !this.sfxEnabled) return;
        if (this.soundsLoaded && this.soundBuffers.bossDestruction) {
            this.playBuffer('bossDestruction', 1.0);
        } else {
            this.playExplosion('large'); // Fallback
        }
    }

    // Scatter laser - for spread shot weapon
    playScatterLaser() {
        if (!this.enabled || !this.sfxEnabled) return;
        if (this.soundsLoaded && this.soundBuffers.scatterLaser) {
            this.playBuffer('scatterLaser', 0.75);
        } else {
            this.playShortLaser(); // Fallback
        }
    }
}
